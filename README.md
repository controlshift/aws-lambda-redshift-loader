# A Fork of AWS Lambda Based Amazon Redshift Database Loader

Instead of responding to S3 events this fork has been modified to respond to SQS queue events
generated by our ControlShift bulk data API Lambda Webhook receiver.

To deploy new versions:

* Increment the version number in package.json
* run `./bin/build.sh` to generate a zip file suitable for distribution.
* run `./bin/deploy.sh` to copy the file to s3.

In our environment, we deploy this new version into our AWS account by applying this terraform module which depends on
the dist archive on s3: https://github.com/controlshift/aws-lambda-redshift-loader
...
- [A Fork of AWS Lambda Based Amazon Redshift Database Loader](#a-fork-of-aws-lambda-based-amazon-redshift-database-loader)
